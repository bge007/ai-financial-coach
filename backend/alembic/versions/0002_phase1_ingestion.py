"""phase 1: uploaded_files, transactions, financial_profiles
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint("user_id", "sha256", name="uq_uploaded_files_user_sha256"),
    )
    ...
    op.create_table(
        ...
        sa.Column("source_file", sa.String(length=512), nullable=False, server_default=""),
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
            server_default=sa.func.now(),
    ...
    op.create_index("ix_transactions_uploaded_file_id", "transactions", ["uploaded_file_id"])
...
def downgrade() -> None:
    ...
    op.drop_index("ix_transactions_uploaded_file_id", table_name="transactions")
